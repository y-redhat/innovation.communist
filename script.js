...
async function decryptPyCipher(b64,password){
    ...
    async function pbkdf2(pwd,salt,len){
...
/* ============================================================
   6) お問い合わせフォーム（保存／表示のみ）
============================================================ */

async function decodeCredentials(d){ return d } // ←後で暗号化対応可
async function encodeCredentials(d){ return d }
...
async function updateContactList(){
    ...
    if(list) list.innerHTML=data.map(c=>
        `<li><b>${c.name}</b> (${c.email}) : ${c.message}</li>`
    ).join("");
}
...
if(contactForm){
    contactForm.addEventListener("submit",async(e)=>{
        ...
        list.push({
            ...
            message:contactForm.message.value   // ←ここ修復！
        });

        localStorage.setItem("contacts", await encodeCredentials(JSON.stringify(list)));
        alert("お問い合わせ送信完了！");
        updateContactList();
    });
}
